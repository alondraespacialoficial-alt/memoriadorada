...
export async function fetchSettingsFromSupabase(): Promise<SiteSettings | null> {
  ...
  try {
    ...
    return {
      ...
      beforeAfterOriginalUrl: data.before_original_url || undefined,
      beforeAfterRestoredUrl: data.before_restored_url || undefined,
...
export async function saveSettingsToSupabase(settings: SiteSettings): Promise<boolean> {
  ...
  try {
    const row = {
      ...
      before_original_url: settings.beforeAfterOriginalUrl ?? null,
      before_restored_url: settings.beforeAfterRestoredUrl ?? null,
